		...
		if (canAddPlayerPosition(position_picked)) {
			...
			if (!Player_Exist(stoi(id))) {
				...
				if (Player_Option == "Y" || Player_Option == "y") {
					...
					if (canAddPlayerPrice(FPLSys::AllPlayers[position_picked][stoi(id)])) {

						if (canAddPlayerCount(FPLSys::AllPlayers[position_picked][stoi(id)])) {

							pickPlayer(FPLSys::AllPlayers[position_picked][stoi(id)]);
...
int User_Team::calculateSquadPoints()
{
	...
	if(squadPerweek.find(FPLSys::CurrGameWeek - 1)!=squadPerweek.end()){
		for (auto player : squadPerweek[FPLSys::CurrGameWeek - 1].first)
		{

			total_points += FPLSys::AllPlayers[player.first][player.second]->CalculatePoints();
		}
...
int User_Team::getTotalPointsPerWeek(int week)
{
	...
	for (auto& player : squadPerweek[week].first) {
		points += FPLSys::AllPlayers[player.first][player.second]->CalculatePoints(week);
	}
		...
		if (!squadPerweek.empty()) {
			...
			for (auto& player : squadPerweek[week].first) {
				cout << "----- " << player.first << " ------" << endl; /// postion
					...
					cout << player.second << " - "
					<< FPLSys::AllPlayers[player.first][player.second]->getFullname() << "\t"
					<< FPLSys::AllPlayers[player.first][player.second]->CalculatePoints(week)
					<< " Points\n";
		...
		for (auto& t : Squad) {
			...
			for (auto& p : t.second) {
			   ...
			   // fixtures in All fixtures is 1 based
				Fixture* f = FPLSys::AllFixtures[FPLSys::CurrGameWeek][FPLSys::AllClubs[p.second->getClub()]->getFixtures()[FPLSys::CurrGameWeek - 1].second];
				cout << p.second->getID() << " - " << p.second->getFullname() << "\t" << " Next Fixture: ";
				
				if (p.second->getClub() == FPLSys::getClubByID(f->getHomeTeam())) {
					cout << FPLSys::getClubByID(f->getAwayTeam())<<endl;
				}
				else {
					cout << FPLSys::getClubByID(f->getHomeTeam()) << endl;
				}
			}
		}
	}
...
{
	displaySquad(FPLSys::CurrGameWeek-1);
}
...
vector<pair<string, int>> User_Team::getWeekSquad(int week = FPLSys::CurrGameWeek-1)
{
	if (week == FPLSys::CurrGameWeek) {
...
void User_Team::StartNewGameWeek()
{
	...
	squadPerweek[FPLSys::CurrGameWeek - 1].second += calculateSquadPoints();
	this->increaseTransfers();
	squadPerweek.insert({ FPLSys::CurrGameWeek,{squadIds,currPoints} });
	currPoints = 0;
}
...
int User_Team::displayGameweeks()
{
	...
	gameweekInput:
		...
		cin >> gameweekChoice;
		if (cin.fail())
			FPLSys::InputFaliure(gameweekChoice, "Please enter your choice");
