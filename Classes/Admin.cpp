...
bool Admin::setPhoneNumber(string phone_number) {
	...
	vPhone = FPLSys::Check_Phone(phone_number);
	if (vPhone) {
...
void Admin::setEmail(string email)
{
	...
	vEmail = FPLSys::Check_Email(email);
	if (vEmail) {
		this->email = email;
...
void Admin::setUsername(string username)
{
	...
	vUserName = FPLSys::Check_Username(username);
	if (vUserName) {
...
void Admin::edit_player_menu()
{
	...
	FPLSys::displayPlayers(position_picked);
	string id;
	...
	FPLSys::displayPlayers(FPLSys::AllPlayers[position_picked][stoi(id)], false, "\n");
	// ASK what change i want make change in temp var
	string Player_Option;
	cout << "Are you sure you want to pick " << FPLSys::AllPlayers[position_picked][stoi(id)]->getFullname() << " ?\n";
	cin >> Player_Option;
	if (Player_Option == "Y" || Player_Option == "y") {
		...
		cin >> option;
		//switch
		option_choise(option, FPLSys::AllPlayers[position_picked][stoi(id)]);
		//	FPLSys::AllPlayers["GKP"][id];
	}
	else{
		edit_player_menu();
	}
...
void Admin::edit_team_menu() {
	...
	DisplayClubs:
		...
		if (FPLSys::AllClubs.find(club_name) == FPLSys::AllClubs.end())
			goto DisplayClubs;

	Display_squad:

		FPLSys::AllClubs[club_name]->DisplaySquad();
		string player_id;

		cout << "Please enter Player ID (or -1 to go back)";
		cin >> player_id;

		unordered_map<int, Player*> squad = FPLSys::AllClubs[club_name]->getSquad();

		if (squad.find(stoi(player_id)) == squad.end())
			goto Display_squad;
		else if (player_id == "-1")
			break;
		FPLSys::AllClubs[club_name]->deletePlayer(stoi(player_id));
...
void Admin::edit_user() { 
		...
		if (FPLSys::AllUsers.find(stoi(user_name)) == FPLSys::AllUsers.end())
			goto display_user;


		
		FPLSys::ChangeAccountSettings(FPLSys::AllUsers[stoi(user_name)]);
}
//===================== Update League Data=========================
...
void Admin::edit_leagues() {
	...
	string confirm;
	string userID;
	FPLSys::displayLeagues(true);
EnterLeagueID:
	cout << "\t\tChoose the League you want to edit or 0 to go back: ";
	cin >> leagueID;
	if (cin.fail())
		FPLSys::InputFaliure(leagueID, "\t\tEnter League you want to edit or 0 to go back");
	if(!FPLSys::isNumber(leagueID))
	{
		cout << "\t\tPlease Enter a League ID in numbers";
		goto EnterLeagueID;
	}
	if (leagueID == "0") return;
	if (FPLSys::AllLeagues.find(stoi(leagueID)) == FPLSys::AllLeagues.end()){
		cout << "\t\tPlease Enter a valid League\n";
		goto EnterLeagueID;
	}
	League* currentleague = FPLSys::AllLeagues[stoi(leagueID)];
	EnterEditChoice:
	...
	cout << "\t\t1- Change league Accessebility\n"
		...
		cin >> choice;
		if (cin.fail())
			FPLSys::InputFaliure(choice, "\t\tEnter League you want to edit");
		if (!FPLSys::isNumber(choice)|| (stoi(choice)>3 && stoi(choice) < 0))
		{
	...
	switch(choice[0]){
		...
		case'3':
			...
			cin >> userID;
			if (cin.fail())
				FPLSys::InputFaliure(userID, "\t\tEnter League you want to edit");
			if (!FPLSys::isNumber(userID))
			{
...
void Admin::edit_fixture_menu()
{
	...
	DisplayFixtures:
		...
		int fixtureID;
		fixtureID=FPLSys::ChooseFixture(goBack);
		if (goBack) return;

	DisplayFixtureClubs:

		string teamName = FPLSys::ChooseFixtureClub(fixtureID, goBack);
		if (goBack)
			goto DisplayFixtures;
SelectFixturePlayer:

		Player* p = FPLSys::ChooseFixturePlayer(teamName, goBack);
		if (goBack)
...
void Admin::startNewGameweek() {
	...
	for (auto& UserTeam : FPLSys::AllUsersTeams) {
