...
string const FPLSys::choice_error = "\tPlease enter your choice here --->\t";
vector<string>  FPLSys::domains= { "gmail.com","outlook.com","yahoo.com","hotmail.com","icloud.com","cis.asu.edu.eg" } ;
string  FPLSys::startchoice="",
        FPLSys:: registerChoice = "",
        FPLSys::loginChoice = "",
        FPLSys::menuChoice = "";
User* FPLSys::CurrUser ;
Admin* FPLSys::CurrAdmin;
int FPLSys::CurrGameWeek = 37;
unordered_map<int,User*> FPLSys::AllUsers;
unordered_map<int,Admin*> FPLSys::AllAdmins;
unordered_map<string, Club*> FPLSys::AllClubs; //name,club object
unordered_map < string, unordered_map<int, Player*>> FPLSys::AllPlayers;
unordered_map <int, User_Team*> FPLSys::AllUsersTeams;
unordered_map<int, League*> FPLSys::AllLeagues; //League id, League object
unordered_map<int, unordered_map<int, Fixture*>>FPLSys::AllFixtures;

void FPLSys::printSeprator() {
    cout << "-------------------------------------------------------------------------------------\n";
    cout << "\033[2J\033[1;1H";
}

void FPLSys::printSeprator_for_errors() {
    cout << "\n-------------------------------------------------------------------------------------\n";

}

void FPLSys::InputFaliure(string& choice, string message) {
    while (cin.fail()) {
        cin.clear();
...
bool FPLSys::Check_Name(string& Name) {
...
bool FPLSys::Check_Database(string&username) {
    string org2 = "";
...
bool FPLSys::Check_Username(string& Username) {
...
bool FPLSys::Check_Password(string& Password)
{
    bool capital = false;
...
bool FPLSys::Check_PhoneDatabase(string phone) {
...
bool FPLSys::Check_Phone(string& Phone) {
    if (Phone.size() != 11) {
...
bool FPLSys::Check_EmailDatabase(string &Email) {
...
bool FPLSys::Check_Email(string& Email) {
...
void FPLSys::RegisterUser() {
    ...
    cout << "\t\tThank You for registering on our FPLSys!\n";
    printSeprator();
    loginChoice = "2";
    startchoice = "1";
}

void FPLSys::RegisterAdmin() {
...
bool FPLSys::userLogin( string attemptedUsername, string attemptedPassword) {
    for (auto& it : AllUsers) {
...
bool FPLSys::AdminLogin( string attemptedUsername, string attemptedPassword) {
    for (auto& it : AllAdmins) {
        if (it.second->getUsername() == attemptedUsername && it.second->getPassword() == attemptedPassword) {
            CurrAdmin = it.second;
            return true;
...
User_Team& FPLSys::getsquad(int id)
{    
...
User* FPLSys::getUser(int id)
{
    return AllUsers[id];
}

void FPLSys::Pass_Encode(string& pass) {
    char input = '0';
    while (input != '\r') {
        input = _getch();
        if (input == '\b') {
            ...
            do {
                ...
                if (Logout_choice.size() == 1 && isalpha(Logout_choice[0])) {
                    if (Logout_choice[0] == 'Y' || Logout_choice[0] == 'y') {
                        ...
                        loginChoice = "3";
                        cout << "\n--------------------Thank You for using our FPLSys!--------------------\n";
                        Sleep(1000);
                        return;
                        break;
                    }
...
void FPLSys::printUserMenu() {
    ...
    case '6': {
        ...
        while (true) {
            cin >> position_picked;
            if (cin.fail())
                FPLSys::InputFaliure(position_picked, "write a suitable number ");
            if (FPLSys::isNumber(position_picked)) {
        ...
        while (true) {
            cin >> id;
            if (cin.fail())
                FPLSys::InputFaliure(id, "write a suitable number ");
            if (FPLSys::isNumber(id)) {
                if (FPLSys::AllPlayers[position_picked].find(stoi(id)) != FPLSys::AllPlayers[position_picked].end()) {
                    break;
                }
                else {
                ...
                if (loginChoice == "") {
                    if (AllUsers.size() != 0||AllAdmins.size()!=0) {
                        ...
                        A- Edit this function so that when the user enter his username and password
                           the FPLSys will check the database and when it knows who he is (Doc or patient)
            ...
            do {
                ...
                */
                ...
                case '3':
                    ...
                    break;
                case '0':
                    cout << "--------------------Thank You for using our FPLSys!--------------------\n";
...
void FPLSys::displayPlayers(Player* p, bool flag = false, string delim = "\n") {
    /// <summary>
    ...
    FPLSys::printSeprator_for_errors();
...
void FPLSys::DisplayPlayerGameweek(Player *p) {
        FPLSys::printSeprator_for_errors();
    EnterGameweekTodisplay:
        ...
        cin >> gameweekNo;
        if (cin.fail())
            FPLSys::InputFaliure(gameweekNo, "write a suitable number ");
        if (gameweekNo == "0")
            return;
        if (FPLSys::isNumber(gameweekNo) && (stoi(gameweekNo) < 38 && stoi(gameweekNo) > 0)) {
            for (auto& match : playerHist) {
...
void FPLSys::displayClubs()
{

    for (auto club : FPLSys::AllClubs) {
        cout << club.first << endl;
    }

}

void FPLSys::displayPlayers(string position) {

    /// <summary>
    /// Displays all players who play in a certain position
    /// </summary>
    /// <param name="position"></param>
    for (auto& it : FPLSys::AllPlayers[position]) {
        if(it.second!=nullptr)
             displayPlayers(it.second, true, "\t||\t");
    }
}
string FPLSys::getClubByID(int id) {
    for (auto& club : AllClubs) {
        if (club.second->getClubID() == id) return club.second->getName();
    }
}
void FPLSys::readFixtures() {
        ...
        while (getline(file, line))
        {
            ...
            if (row_counter>-1) {
                if (p==nullptr||stoi(row["id_player"].at(row_counter)) != p->getID()) {//initializing
                    ...
                    FPLSys::AllPlayers[p->getPosition()].insert({ p->getID(),p });
                }
                else {//updating
...
string FPLSys::space2underscore(string text) {
...
string FPLSys::underscore2space(string text) {
    for (int i = 0; i < text.size(); i++) {
        if (text[i] == '_')
            text[i] = ' ';
    }
...
void FPLSys::readLeagues()
{
    ...
    if (UserTeamsFile.is_open())
    {
        ...
        while (UserTeamsFile >> DataLine)
        {
            ...
            UserTeamsFile >> DataLine;
            league->setname(FPLSys::underscore2space(DataLine));
            UserTeamsFile >> DataLine;
            league->setIsPublic((bool)stoi(DataLine));
            UserTeamsFile >> DataLine;
            league->setCode(stoi(DataLine));
            UserTeamsFile >> DataLine;//==========LeaderBoard==============
            league->setLeagueCreator(FPLSys::getUser(stoi(DataLine)));
            ...
            while (DataLine != "==========EndLeague==============") {
                ...
                while (getline(str, cellData, ',')) {
                    if (count == 0) {
                        ...
                        User* u = new User();
                        u = FPLSys::getUser(stoi(cellData));
...
void FPLSys::writeLeagues()
{
    ...
    if (Leagues.is_open()) {
        for (auto league = AllLeagues.begin(); league != AllLeagues.end(); league++) {
            ...
            Leagues << FPLSys::space2underscore(league->second->getName()) << endl;
...
void FPLSys::displayLeagues(bool displayAll) {
...
void FPLSys::ChangeAccountSettings(User* CurrUser) {
    string option;
    string changing;
...
void FPLSys::ChangeAccountSettings(Admin *CurrAdmin)
{
    string changing;
    string reenter_password;
    string option;
...
void FPLSys::Transfers() {
    ...
    while (true) {
        cin >> user_option;
        if (cin.fail())
            FPLSys::InputFaliure(user_option, "write a suitable number ");
        if (FPLSys::isNumber(user_option)) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (cin.fail())
                FPLSys::InputFaliure(id, "write a suitable number ");
            if (FPLSys::isNumber(id)) {
                if (FPLSys::AllPlayers[position].find(stoi(id)) != FPLSys::AllPlayers[position].end()) {
                    break;
                }
                else {
                    cout << "Player does not exist\n";
        ...
        if (!Current_Team->Player_Exist(stoi(id))) {
            ...
            FPLSys::displayPlayers(FPLSys::AllPlayers[position][stoi(id)], false, "\n");
            string Player_Option;
            cout << "Are you sure you want to pick " << FPLSys::AllPlayers[position][stoi(id)]->getFullname() << " ?\n";
            cin >> Player_Option;
...
void FPLSys::ManageSqaudMenu(User_Team& c) {
...
void FPLSys::ViewPlayers() { // Last gameweek SQUAD
    User_Team* Current_Team = AllUsersTeams[CurrUser->getID()];
    int weekNo;
...
void FPLSys::displayFixtures(int week) {
...
int FPLSys::ChooseFixture(bool&goBack) {
    string choice;
...
string FPLSys::ChooseFixtureClub(int fixtureID,bool& goBack) {
    string TeamName;
    unordered_map<int, string>clubs = getClubsByID();
...
Player* FPLSys::ChooseFixturePlayer(string teamName,bool &goBack)
{
    EnterPlayerId:
        string choice;
...
unordered_map<int, string>FPLSys::getClubsByID() {
...
void FPLSys::writeFixtures(){
	...
	if (FixtureFile.is_open())
	{
		...
		for (auto& GameweekMatches : FPLSys::AllFixtures)
		{
...
void FPLSys::writeFPLSysconfig()
{
    fstream FPLSysFile;
    FPLSysFile.open("Config.txt", ios::out);

    if (FPLSysFile.is_open())
    {
        FPLSysFile << CurrGameWeek<<endl;
        FPLSysFile << "================StartDomains==================\n";
        for (auto& domain : domains)
            FPLSysFile << domain << endl;
        FPLSysFile << "================EndDomains==================\n";
    }

    FPLSysFile.close();
}
void FPLSys::ReadFPLSysconfig()
{
    vector<string> d;
    fstream FPLSysfile("Files\\Config.txt", ios::in);
    
    if (FPLSysfile.is_open())
    {
        string DataLine;
        while (FPLSysfile >> DataLine)
        {
            
            CurrGameWeek = stoi(DataLine);
            FPLSysfile >> DataLine;//"================StartDomains==================\n"
            FPLSysfile >> DataLine;
