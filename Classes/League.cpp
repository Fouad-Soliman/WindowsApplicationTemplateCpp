...
void League::UpdateLeaderBoard()
{
	...
	for (int i = 0; i < leaderBoard.size(); i++) {
		newleaderboard.push({ (FPLSys::AllUsersTeams[(p + i)->second.first]->getTotalPointsPerWeek(FPLSys::CurrGameWeek - 1) + (p + i)->second.second->getTotalPoints()) , {(p + i)->second.first,(p + i)->second.second}});
	}
